...
export default function PortfolioDetailPage() {
  ...
  React.useEffect(() => {
    async function fetchPortfolio() {
      try {
        const data = await api.getPortfolioById(portfolioId);
        setPortfolio(data);
      } catch (err) {
  ...
  const contentItems = portfolio.contentItems || portfolio.contents || [];
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  <span className="text-sm font-medium text-gray-600 min-w-[100px]">Content:</span>
                  <span className="text-sm text-gray-900">{contentItems.length} items</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {portfolio.profile && (
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    <span className="text-sm font-medium text-gray-600 min-w-[100px] flex items-center gap-2">
                      <UserCircle className="h-4 w-4" />
                      Student:
                    </span>
                    <div className="flex-1">
                      <div className="text-sm text-gray-900">{portfolio.profile.name}</div>
                      {portfolio.profile.user && (
                        <div className="text-xs text-gray-500">{portfolio.profile.user.email}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {portfolio.profile.institutionClass && (
                    <div className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-900">
                        {portfolio.profile.institutionClass.name}
                      </span>
                    </div>
                  )}
                  ...
                  {portfolio.profile.institution && (
                    <div className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-900">
                        {portfolio.profile.institution.name}
                      </span>
                    </div>
                  )}
                </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-gray-900">
                                {itemTitle || `${contentTypeLabels[item.type]} ${index + 1}`}
                              </span>
                            ...
                            {item.type === "AUDIOBOOK" && item.coverPageUrl && (
                              <div className="mt-2">
                                <img
                                  src={item.coverPageUrl}
                                  alt={itemTitle || "Audiobook cover"}
                                  className="h-20 w-20 object-cover rounded"
                                />
                              </div>
                            )}
